...
foreach x of varlist HorvathAgeEAA_no_outliers HannumAgeEAA_no_outliers DunedinPoAm_no_outliers telomean_no_outliers zHorvathAgeEAA zHannumAgeEA zDunedinPoAm ztelomean_no_outliers {
...
(zHorvathAgeEAA -> zHannumAgeEA, family(gaussian) link(identity)) ///
...
(SEX -> zGrimAgeMortEAA, family(gaussian) link(identity)) ///
(NHB -> zDunedinPoAm, family(gaussian) link(identity)) ///
(NHB -> ztelomean_no_outliers, family(gaussian) link(identity)) ///
(HISP -> zHannumAgeEAA, family(gaussian) link(identity)) ///
(OTHER -> zPhenoAgeEAA, family(gaussian) link(identity)) ///
(zHorvathAgeEAA -> zDunedinPoAm, family(gaussian) link(identity)) ///
